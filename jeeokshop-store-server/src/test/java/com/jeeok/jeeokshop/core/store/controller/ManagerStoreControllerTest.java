...
class ManagerStoreControllerTest {
    ...
    void findStores() throws Exception {
        //given
        List<Category> categories = new ArrayList<>();
        IntStream.range(0, 2).forEach(i -> categories.add(getCategory(CATEGORY_NAME + i, ORDER + i)));

        List<Store> stores = new ArrayList<>();
        IntStream.range(0, 5).forEach(i -> stores.add(getStore(STORE_NAME + i, BUSINESS_HOURS, PHONE_NUMBER, ADDRESS, MEMBER_ID, categories)));
        given(storeService.findStores(any(StoreSearchCondition.class), any(Pageable.class))).willReturn(new PageImpl<>(stores));
        ...
        //verify
        verify(storeService, times(1)).findStores(any(StoreSearchCondition.class), any(Pageable.class));
    }
    ...
    void findStore() throws Exception {
        //given
        given(storeService.findStore(any(Long.class))).willReturn(store);

        //expected
        mockMvc.perform(get(API_FIND_STORE, STORE_ID))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(Code.SUCCESS.name()))
                .andExpect(jsonPath("$.message").isEmpty())
                .andExpect(jsonPath("$.errors").isEmpty())
        ...
        //verify
        verify(storeService, times(1)).findStore(any(Long.class));
    }
    ...
    void saveStore() throws Exception {
        //given
        given(storeService.saveStore(any(SaveStoreParam.class))).willReturn(store);
        ...
        //expected
        mockMvc.perform(post(API_SAVE_STORE)
        ...
        //verify
        verify(storeService, times(1)).saveStore(any(SaveStoreParam.class));
    }
    ...
    void updateStore() throws Exception {
        ...
        //expected
        mockMvc.perform(put(API_UPDATE_STORE, STORE_ID)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(Code.SUCCESS.name()))
        ...
        //verify
        verify(storeService, times(1)).updateStore(any(Long.class), any(UpdateStoreParam.class));
        verify(storeService, times(1)).findStore(any(Long.class));
    }
    ...
    void deleteStore() throws Exception {
        ...
        //expected
        mockMvc.perform(delete(API_DELETE_STORE, STORE_ID))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(Code.SUCCESS.name()))
                .andExpect(jsonPath("$.message").isEmpty())
                .andExpect(jsonPath("$.errors").isEmpty())
